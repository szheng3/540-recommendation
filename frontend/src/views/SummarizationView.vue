...
const selectedToken = computed(() => {
  return models.value.find((model) => model.name === defaultSelected.value)
    .token;
});
    ...
    try {
      const response = await axios.post("/api/summary", {
        ...
        minlength: Math.round(
          (sliderValue.value / 100) * textInput.value.split(" ").length
        ),
        ...
        <v-sheet rounded class="mt-4">
          <v-form ref="myForm" @submit.prevent="submitForm" class="pa-3">
            ...
            <div class="text-caption">
              ...
              <strong>{{
                Math.round((sliderValue / 100) * textInput.split(" ").length)
              }}</strong>
            </div>
            <v-slider
              v-model="sliderValue"
